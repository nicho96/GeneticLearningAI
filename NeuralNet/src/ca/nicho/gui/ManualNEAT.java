...
public class ManualNEAT extends DefaultNEAT2 {
	...
	private JButton mutateB = new JButton("Mutate Current Network");
	...
	private void initControls(){
		...
		infoB.setBounds(10, 90, 200, 30);
		infoB.addActionListener(printInfo);
		controls.add(infoB);
		
		mutateB.setBounds(10, 140, 200, 30);
		mutateB.addActionListener(mutateCurrent);
		controls.add(mutateB);
	...
	private ActionListener printInfo = (ActionEvent ev) -> {	
		System.out.println(screen.getNetwork());
	};
	
	private ActionListener mutateCurrent = (ActionEvent ev) -> {	
		this.mutate(screen.getNetwork());
		screen.setNeuralNetwork(screen.getNetwork());
	};
